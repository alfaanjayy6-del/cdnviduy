...
export default function Home() {
  ...
  // Fungsi cek apakah video diupload kurang dari 24 jam yang lalu
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', backgroundColor: '#111', color: '#fff', minHeight: '100vh' }}>
      ...
      <Script src="https://pl27333728.effectivegatecpm.com/e4/6a/bf/e46abf385099c2b5d894dbb1c522e30c.js" strategy="lazyOnload" />

      {/* Tombol Filter */}
      <div style={{ textAlign: 'center', marginBottom: '30px', display: 'flex', justifyContent: 'center', gap: '10px' }}>
        <button 
          onClick={() => fetchVideos('terbaru')}
          style={{ padding: '8px 15px', borderRadius: '20px', border: 'none', cursor: 'pointer', backgroundColor: filter === 'terbaru' ? '#f00' : '#333', color: '#fff' }}
        >
          ✨ Terbaru
        </button>
        <button 
          onClick={() => fetchVideos('abjad')}
          style={{ padding: '8px 15px', borderRadius: '20px', border: 'none', cursor: 'pointer', backgroundColor: filter === 'abjad' ? '#f00' : '#333', color: '#fff' }}
        >
          🔠 A-Z
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px', maxWidth: '1200px', margin: '0 auto' }}>
        {videos.map((vid) => (
          <div key={vid.id} style={{ border: '1px solid #333', padding: '15px', borderRadius: '12px', backgroundColor: '#1a1a1a', position: 'relative' }}>
            ...
            {/* Badge NEW jika video diupload < 24 jam */}
            ...
            <button onClick={() => shareLink(vid.videy_id)} style={{ marginTop: '15px', width: '100%', padding: '10px', backgroundColor: '#333', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
              🔗 Salin Link Share
            </button>
          </div>
        ))}
